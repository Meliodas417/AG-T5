...
For installing and setting up the KPI Formula Tool, follow these steps:
...
Initialize the `DataManager` for data operations:
...
Example:
Refer to `example_db_usage.py` and `create_test_db.py` for usage examples.

Note: Run `create_test_db.py` first!
